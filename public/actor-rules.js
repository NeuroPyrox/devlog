...
const genInteractionRule = (
  ...
  const sendCommands = Array.from({ length: genNumSendCommands() }, () =>
    genSendCommand(localRefGen)
  );
  const becomeCommand = genBecomeCommand(
    localRefGen,
    getRandomBehavior,
    getNumBehaviorRefs
  );
...
export default () => {
  ...
  const numBehaviorRefs = Array.from(
    { length: numBehaviors },
    genNumBehaviorRefs
  );
  ...
  return behaviorRules.map((rules, index) => ({
    size: getNumBehaviorRefs(index),
    rules: rules,
  }));
};
